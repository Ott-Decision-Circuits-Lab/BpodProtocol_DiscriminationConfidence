...
if BpodSystem.EmulatorMode
    ...
    StopLaserWaveform = {'WavePlayer1', ['P' 10]};  %play the 11th profile
    EarlyWithdrawalSoundAndStopLaser = {'WavePlayer1', ['P' 11]}; %play the 12th profile
end
...
if BpodSystem.Data.Custom.TrialData.AuditoryTrial(iTrial)
    if BpodSystem.Data.Custom.TrialData.ClickTask(iTrial)
        if TaskParameters.GUI.TonicProtocol && BpodSystem.Data.Custom.TrialData.TonicLaserTrial(iTrial)==0
            sma = AddState(sma, 'Name', 'stay_Cin',...
                ...
                'OutputActions', StopLaserWaveform); %'BNCState',BNC2OutPre});
        else
            sma = AddState(sma, 'Name', 'stay_Cin',...
                'Timer', TaskParameters.GUI.StimDelay,...
                'StateChangeConditions', {CenterPortOut,'broke_fixation','Tup', 'stimulus_delivery_min'},...
        ...
        if (TaskParameters.GUI.LaserStim && BpodSystem.Data.Custom.TrialData.PhasicLaserTrial(iTrial)) || (TaskParameters.GUI.TonicProtocol && BpodSystem.Data.Custom.TrialData.TonicLaserTrial(iTrial))
            sma = AddState(sma, 'Name', 'stimulus_delivery_min',...
                ...
                'OutputActions', ClicksAndLaserStimulation); %'BNCState',1+BNC2OutST});
        else
            sma = AddState(sma, 'Name', 'stimulus_delivery_min',...
                'Timer', TaskParameters.GUI.MinSampleAud,...
                'StateChangeConditions', {CenterPortOut,'early_withdrawal','Tup','stimulus_delivery'},...
        ...
        if TaskParameters.GUI.LaserStim && BpodSystem.Data.Custom.TrialData.PhasicLaserTrial(iTrial)
            sma = AddState(sma, 'Name', 'early_withdrawal',...
                'Timer',0,...
                'StateChangeConditions',{'Tup','timeOut_EarlyWithdrawal'},...
                'OutputActions', EarlyWithdrawalSoundAndStopLaser);
        else 
            sma = AddState(sma, 'Name', 'early_withdrawal',...
                'Timer',0,...
                'StateChangeConditions',{'Tup','timeOut_EarlyWithdrawal'},...
...
if BpodSystem.Data.Custom.TrialData.PhasicLaserTrial(iTrial) && TaskParameters.GUI.LaserStim
    sma = AddState(sma, 'Name','start_Lin2',...
        'Timer',0,...
        'StateChangeConditions', {'Tup',LeftPokeAction},...
        'OutputActions',[{'GlobalTimerTrig',2}, StopLaserWaveform]);
...
if BpodSystem.Data.Custom.TrialData.PhasicLaserTrial(iTrial) && TaskParameters.GUI.LaserTimeInvestment
    sma = AddState(sma, 'Name', 'rewarded_Lin_start',...
        'Timer', 0.05,...
        'StateChangeConditions', {LeftPortOut,'rewarded_Lin_grace','Tup','rewarded_Lin'},...
        'OutputActions', LaserStimulation); %[Wire1OutCorrect, {'BNCState',BNC2OutWT}]);
    ...
    sma = AddState(sma, 'Name', 'unrewarded_Lin_start',...
        ...
        'OutputActions', LaserStimulation); %[Wire1OutError, {'BNCState',BNC2OutWT}]); 
    ...
    sma = AddState(sma, 'Name', 'water_L',...
        ...
        'OutputActions', [{'ValveState', LeftValve} , StopLaserWaveform]); %,'BNCState',BNC2OutReward});
    sma = AddState(sma, 'Name', 'water_R',...
        'Timer', RightValveTime,...
        'StateChangeConditions', {'Tup','DrinkingR'},...
        'OutputActions', [{'ValveState', RightValve}, StopLaserWaveform]); %,'BNCState',BNC2OutReward});

else    %no laser stimulation during TI
    sma = AddState(sma, 'Name', 'rewarded_Lin_start',...
        'Timer', 0.05,...
        'StateChangeConditions', {LeftPortOut,'rewarded_Lin_grace','Tup','rewarded_Lin'},...
...
sma = AddState(sma, 'Name', 'unrewarded_Lin_grace',...
    ...
    'OutputActions', {}); %'BNCState',BNC2OutWT}); <- not necessary
...
if BpodSystem.Data.Custom.TrialData.PhasicLaserTrial(iTrial) && TaskParameters.GUI.LaserTimeInvestment
    if  TaskParameters.GUI.IncorrectChoiceFeedbackType == 2 % IncorrectChoiceFeedbackType == Tone
        sma = AddState(sma, 'Name', 'timeOut_IncorrectChoice',...
            ...
            'StateChangeConditions',{'Tup','ITI'},...
            'OutputActions', IncorrectChoiceSound, StopLaserWaveform); %'SoftCode',11,'BNCState',BNC2OutFB});
    elseif  TaskParameters.GUI.IncorrectChoiceFeedbackType == 3 % IncorrectChoiceFeedbackType == PortLED
        ...
        sma = AddState(sma, 'Name', 'timeOut_IncorrectChoice2',...
            'Timer',TaskParameters.GUI.TimeOutIncorrectChoice,... 
            'StateChangeConditions',{'Tup','ITI'},...
            'OutputActions',StopLaserWaveform); %'BNCState',BNC2OutFB});
    ...
    'OutputActions', StopLaserWaveform);

else    % no laser TI stim protocol
    if  TaskParameters.GUI.IncorrectChoiceFeedbackType == 2 % IncorrectChoiceFeedbackType == Tone
        sma = AddState(sma, 'Name', 'timeOut_IncorrectChoice',...
            'Timer',TaskParameters.GUI.TimeOutIncorrectChoice,...
            'StateChangeConditions',{'Tup','ITI'},...
