...
LeftValveTimeInterrupt = LeftValveTime*TaskParameters.GUI.InterruptRewPer;
RightValveTimeInterrupt = RightValveTime*TaskParameters.GUI.InterruptRewPer;
...
if  BpodSystem.Data.Custom.LaserTrial(iTrial) %laser trial. BNC2 to high (1 still low).
    ...
    if TaskParameters.GUI.LaserInterrupt
    BNC2OutInterrupt = 2;%interrupt feedback states (interrupt delays)
    end  
...
if BpodSystem.Data.Custom.InterruptDelay(iTrial) && BpodSystem.Data.Custom.CatchTrial(iTrial)
    FeedbackDelayCorrect = BpodSystem.Data.Custom.InterruptDelayStartTime(iTrial);
    rewarded_Lin_Tup_action='Interrupt_L_start';
    rewarded_Rin_Tup_action='Interrupt_R_start';
else
    rewarded_Lin_Tup_action='water_L';
    rewarded_Rin_Tup_action='water_R';
end
%percentage of error trials (only when on catch)
if BpodSystem.Data.Custom.InterruptDelay(iTrial) && TaskParameters.GUI.CatchError
    FeedbackDelayError = BpodSystem.Data.Custom.InterruptDelayStartTime(iTrial);
    unrewarded_Lin_Tup_action = 'Interrupt_L_start';
    unrewarded_Rin_Tup_action = 'Interrupt_R_start';
else
    unrewarded_Lin_Tup_action = 'timeOut_IncorrectChoice';
    unrewarded_Rin_Tup_action = 'timeOut_IncorrectChoice';
end
...
sma = SetGlobalTimer(sma,3,BpodSystem.Data.Custom.InterruptDelayTime(iTrial));
...
sma = AddState(sma, 'Name', 'rewarded_Lin',...
    ...
    'StateChangeConditions', {LeftPortOut,'rewarded_Lin_grace','Tup',rewarded_Lin_Tup_action,'GlobalTimer1_End',rewarded_Lin_Tup_action},...
...
sma = AddState(sma, 'Name', 'rewarded_Rin',...
    'Timer', FeedbackDelayCorrect,...
    'StateChangeConditions', {RightPortOut,'rewarded_Rin_grace','Tup',rewarded_Rin_Tup_action,'GlobalTimer1_End',rewarded_Rin_Tup_action},...
    'OutputActions', {'BNCState',BNC2OutWT});
...
sma = AddState(sma, 'Name', 'unrewarded_Lin',...
    ...
    'StateChangeConditions', {LeftPortOut,'unrewarded_Lin_grace','Tup',unrewarded_Lin_Tup_action,'GlobalTimer2_End',unrewarded_Lin_Tup_action},...
    'OutputActions', {'BNCState',BNC2OutWT}); 
...
sma = AddState(sma, 'Name', 'unrewarded_Rin',...
    'Timer', FeedbackDelayError,...
    'StateChangeConditions', {RightPortOut,'unrewarded_Rin_grace','Tup',unrewarded_Rin_Tup_action,'GlobalTimer2_End',unrewarded_Rin_Tup_action},...
    'OutputActions', {'BNCState',BNC2OutWT});
...
sma = AddState(sma, 'Name', 'water_Interrupt_L',...
    'Timer', LeftValveTimeInterrupt,...
    'StateChangeConditions', {'Tup','ITI'},...
    'OutputActions', {'ValveState', LeftValve,'BNCState',BNC2OutReward});
sma = AddState(sma, 'Name', 'water_Interrupt_R',...
    'Timer', RightValveTimeInterrupt,...
    'StateChangeConditions', {'Tup','ITI'},...
    'OutputActions', {'ValveState', RightValve,'BNCState',BNC2OutReward});
...
%interrupt delay system
sma = AddState(sma, 'Name', 'Interrupt_L_start',...
    'Timer', 0,...
    'StateChangeConditions', {'Tup','Interrupt_L'},...
    'OutputActions', {'GlobalTimerTrig',3}); 
sma = AddState(sma, 'Name', 'Interrupt_L',...
    'Timer', BpodSystem.Data.Custom.InterruptDelayTime(iTrial),...
    'StateChangeConditions', {LeftPortOut,'Interrupt_L_grace','Tup','water_Interrupt_L','GlobalTimer3_End','water_Interrupt_L'},...
    'OutputActions', {'BNCState',BNC2OutInterrupt,strcat('PWM',num2str(LeftPort)),255}); 
sma = AddState(sma, 'Name', 'Interrupt_L_grace',...
    'Timer', TaskParameters.GUI.FeedbackDelayGrace,...
    'StateChangeConditions', {'Tup','skipped_feedback_Interrupt','GlobalTimer3_End','skipped_feedback_Interrupt',LeftPortIn,'Interrupt_L'},...
    'OutputActions', {'BNCState',BNC2OutInterrupt,strcat('PWM',num2str(LeftPort)),255}); 
sma = AddState(sma, 'Name', 'Interrupt_R_start',...
    'Timer', 0,...
    'StateChangeConditions', {'Tup','Interrupt_R'},...
    'OutputActions', {'GlobalTimerTrig',3}); 
sma = AddState(sma, 'Name', 'Interrupt_R',...
    'Timer', BpodSystem.Data.Custom.InterruptDelayTime(iTrial),...
    'StateChangeConditions', {RightPortOut,'Interrupt_R_grace','Tup','water_Interrupt_R','GlobalTimer3_End','water_Interrupt_R'},...
    'OutputActions', {'BNCState',BNC2OutInterrupt,strcat('PWM',num2str(RightPort)),255}); 
sma = AddState(sma, 'Name', 'Interrupt_R_grace',...
    'Timer', TaskParameters.GUI.FeedbackDelayGrace,...
    'StateChangeConditions', {'Tup','skipped_feedback_Interrupt','GlobalTimer3_End','skipped_feedback_Interrupt',RightPortIn,'Interrupt_R'},...
    'OutputActions', {'BNCState',BNC2OutInterrupt,strcat('PWM',num2str(RightPort)),255});
sma = AddState(sma, 'Name', 'skipped_feedback_Interrupt',...
    'Timer', 0,...
